...
function  checkuser() {
    ...
    if(username == ""){
        $("#icon_username").html("");
        $("#icon_username").html("<i class='layui-icon layui-icon-close-fill'></i>");
        layer.alert("账号不能为空",{icon:0,title:'新增用户提示',time:2500});
    }else {
        $.ajax({
            url: "/userAdmin/checkuser",
            ...
            success: function (result, textStatus, request) {
                ...
                if (result.code == 100) {
                    $("#icon_username").html("");
                    $("#icon_username").html("<i class='layui-icon layui-icon-ok-circle'></i>");
                }else if (result.code == 200){
                    layer.msg("验证账号失败",{icon:5});
                } else if (result.code == 210) {
                    layer.alert("您输入的账户已存在", {icon: 0, title: '新增用户提示', time: 25000});
                    $("#icon_username").html("");
                    $("#icon_username").html("<i class='layui-icon layui-icon-close-fill'></i>");
...
function checkpassword() {
    ...
    if (ValueIsEmpty(password)){
        $("#icon_password").html("");
        $("#icon_password").html("<i class='layui-icon layui-icon-close-fill'></i>");
        layer.alert("密码不能为空",{icon:0,title:'新增用户提示',time:2500});
    }else {
        $("#icon_password").html("");
        $("#icon_password").html("<i class='layui-icon layui-icon-ok-circle'></i>");
    }
}
//验证姓名是否为空
...
function makesureinsert() {
    ...
    if (ValueIsEmpty(username)){
        $("#icon_username").html("");
        $("#icon_username").html("<i class='layui-icon layui-icon-close-fill'></i>");
        layer.alert("角色名称不能为空",{icon:0,title:'提示',time:2500});
        return false;
    }else {
        $("#icon_username").html("");
        $("#icon_username").html("<i class='layui-icon layui-icon-ok-circle'></i>");
    ...
    $.ajax({
        ...
        data: {"username":username,"password":password,"realname":realname,"sex":sex,"dwcode":dwcode,"bmid":bmid,"roleid":roleid,"perms":perms,"phone":phone,"photo":photo,"delflag":del},
        type: "post",
        beforeSend: function (request) {
            request.setRequestHeader("Token", GetToken("Token"));
        },
        success: function (result, textStatus, request) {
            ...
            SetToken("Token", token);
            if (result.code == 100) {
               layer.alert("新增用户成功",{icon: 0, title: '新增用户提示', time: 25000},function () {
                   parent.layer.closeAll();
               })
            } else if (result.code == 210) {
                layer.alert("您输入的用户已存在", {icon: 0, title: '新增用户提示', time: 25000});
                $("#form_icon").html("");
                $("#form_icon").html("<i class='layui-icon layui-icon-close-fill'></i>");
            }else if (result.code == 200){
                layer.alert("新增用户失败",{icon: 0, title: '新增用户提示', time: 25000});
            }else if(result.code == 230){
                layer.alert(result.msg,{icon: 0, title: '新增用户提示', time: 25000});
            }
            else if (result.code == 320) {
                top.layer.alert("会话已过期，请您重新登录", {icon: 0, title: '会话过期提示', time: 2500}, function () {
                    top.window.location.href = "/login";
