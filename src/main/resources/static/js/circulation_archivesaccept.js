...
function selectinfobyslh() {
    ...
    $.ajax({
        url: "/circulation/selectinfobyslh",
        type: 'post',
        data:{"slh":slh},
...
function initTable(result) {
    ...
    var html = "<tr id='"+info.slh+"'>"+
        ...
        "<td style='word-wrap: break-word;word-break: break-all;'>"+(info.qymc==undefined?'':info.qymc)+"</td>"+
...
function sureaccept() {
    ...
    var slh = getSLH();
    $.ajax({
        url: "/circulation/updateslxxofslzt",
        type: 'post',
        data:{"slh":slh},
        beforeSend: function (request) {
            request.setRequestHeader("Token",GetToken("Token"));
