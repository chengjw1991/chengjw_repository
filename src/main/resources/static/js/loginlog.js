...
function init(pageindex,pagecount) {
    ...
    $.ajax({
        url: "/systemLog/selectloginlogduringdate",
        type: "post",
...
function datatable(data){
    ...
    $.each(data,function (i,item) {
        html += "<tr>"+
            ...
            "<td>"+(item.dwname==undefined?'':item.dwname)+"</td>"+
            "<td>"+(item.dwcode==undefined?'':item.dwcode)+"</td>"+
...
function selectloginlogbyname() {
    init(1,12);
}
//对查询的数据按照日期进行升序排序
function upsortbydate() {
    var username = $("input[name='username_hidden']").val();
    var startdate = $("input[name='startdate_hidden']").val();
    var enddate = $("input[name='enddate_hidden']").val();
}